...
def ingest(download):
    with mlflow.start_run(run_name="housing_data_ingestion", nested=True) as child_run:
        ...
        # Log data size
        mlflow.log_metric("initial_data_rows", housing.shape[0])
        mlflow.log_metric("initial_data_columns", housing.shape[1])
